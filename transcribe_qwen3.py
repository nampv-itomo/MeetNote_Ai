...
transcribe_qwen3.py
-------------------
Dùng Qwen3-ASR-1.7B (CrispASR + Vulkan) để chuyển audio thành văn bản tiếng Việt.

Ưu điểm:
  - Chính xác cao cho tiếng Việt, có dấu
  - Chạy trên AMD GPU (Vulkan) — nhanh hơn CPU nhiều
  - Tự động thêm dấu câu, viết hoa
  - Không cần PyTorch, nhẹ hơn Whisper

Hạn chế:
  - Không đọc được M4A trực tiếp — cần convert sang WAV 16kHz mono bằng ffmpeg
  - Phụ thuộc vào qwen3-asr-stt wrapper

Model path: /run/media/viper/Data/models/qwen3-asr-0.6b/qwen3-asr-1.7b-q4_k.gguf
...
from load_env import QWEN3_ASR_MODEL_17B
QWEN3_MODEL = QWEN3_ASR_MODEL_17B
# Legacy path (fallback)
QWEN3_MODEL_FALLBACK = "/run/media/viper/Data/models/qwen3-asr-0.6b/qwen3-asr-1.7b-q4_k.gguf"
LANG = "vi"
TIMEOUT = 300  # giây
...
def transcribe_segments(segment_paths: list[str], language: str | None = None,
    ...
    """
    Transcribe nhiều đoạn audio liên tiếp bằng Qwen3-ASR-1.7B và ghép thành văn bản.

    Args:
        segment_paths: danh sách đường dẫn các đoạn audio (đúng thứ tự).
        language: mã ngôn ngữ (mặc định "vi", không dùng).
